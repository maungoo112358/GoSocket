...
func (m *ConnectionModule) handleHandshake(conn net.PacketConn, addr net.Addr, pkt *gamepacket.GamePacket) {
	...
	tempID := utils.GenerateTempID()
	client.AddPendingClient(tempID, addr)
	...
	if err := utils.SendToAddress(conn, response, addr); err != nil {
...
func (m *ConnectionModule) sendUsernamePrompt(conn net.PacketConn, addr net.Addr, message string) {
	...
	if err := utils.SendToAddress(conn, prompt, addr); err != nil {
...
func (m *ConnectionModule) handleUsernameSubmission(conn net.PacketConn, addr net.Addr, pkt *gamepacket.GamePacket) {
	...
	if valid, errorMsg := utils.IsValidUsername(username); !valid {
...
func (m *ConnectionModule) acceptUsername(conn net.PacketConn, addr net.Addr, seq uint32, username, originalUsername string) {
	privateID := utils.GeneratePrivateID()

	client.RemovePendingClient(addr)

	client.AddClient(privateID, username, originalUsername, addr)
	...
	if err := utils.SendToAddress(conn, handshakeResponse, addr); err != nil {
...
func (m *ConnectionModule) isUsernameAvailable(conn net.PacketConn, username string) bool {
	existingClient := client.FindClientByPublicID(username)
...
func (m *ConnectionModule) isClientStale(c *client.ClientInfo) bool {
	return time.Since(c.LastHeartbeat) >= StaleClientTimeout
}

func (m *ConnectionModule) removeStaleClient(conn net.PacketConn, c *client.ClientInfo, username string) {
	timeSinceHeartbeat := time.Since(c.LastHeartbeat)

	fmt.Printf("🗑️ Removing stale client %s (last heartbeat: %.1fs ago)\n",
		c.PublicID, timeSinceHeartbeat.Seconds())

	client.RemoveClient(conn, c.PrivateID)
...
func (m *ConnectionModule) handleReconnectionRequest(conn net.PacketConn, addr net.Addr, pkt *gamepacket.GamePacket) {
	...
	if restoredClient := m.tryRestoreClient(req.Username, req.SessionToken, addr); restoredClient != nil {
		...
		m.sendReconnectionFailure(conn, addr, pkt.Seq)
		fmt.Printf("❌ Reconnection failed for %s from %s\n", req.Username, addr)
	}
}

func (m *ConnectionModule) sendReconnectionSuccess(conn net.PacketConn, addr net.Addr, seq uint32, c *client.ClientInfo) {
	response := &gamepacket.GamePacket{
		Seq: seq,
...
func (m *ConnectionModule) sendReconnectionFailure(conn net.PacketConn, addr net.Addr, seq uint32) {
	response := &gamepacket.GamePacket{
		Seq: seq,
		ReconnectionResponse: &gamepacket.ReconnectionResponse{
			IsSuccessful: false,
...
func (m *ConnectionModule) tryRestoreClient(username, sessionToken string, addr net.Addr) *client.ClientInfo {
	existingClient := client.FindClientByPublicID(username)

	_, hadLobbyState := client.GetLobbyPosition(username)
...
func (m *ConnectionModule) handleExistingClientReconnection(existing *client.ClientInfo, sessionToken string, addr net.Addr, username string) *client.ClientInfo {
...
func (m *ConnectionModule) replaceStaleClient(existing *client.ClientInfo, sessionToken string, addr net.Addr, username string) *client.ClientInfo {
	// Save important state before removal
	savedColor := existing.ColorHex
	wasInLobby := existing.InLobby

	client.RemoveClient(nil, existing.PrivateID) // Don't broadcast since we're replacing
...
func (m *ConnectionModule) createRestoredClient(username, sessionToken string, addr net.Addr, inLobby bool) *client.ClientInfo {
	privateID := utils.GeneratePrivateID()
	now := time.Now()
...
func (m *ConnectionModule) handleHeartbeat(conn net.PacketConn, addr net.Addr, pkt *gamepacket.GamePacket) {
	...
	if !client.UpdateHeartbeat(hb.ClientId) {
	...
	if err := utils.SendToAddress(conn, ack, addr); err != nil {
		fmt.Printf("❌ Failed to send heartbeat ack to %s: %v\n", addr, err)
	}
}

func (m *ConnectionModule) Shutdown() {
}
